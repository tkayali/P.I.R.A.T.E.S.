...
class PIRATES(ShowBase):
	def __init__(self):
		...
		#self.accept("x", self.printx)
		#self.accept("y", self.printy)
		#self.accept("z", self.printz)
		#self.accept("h", self.printh)
		#self.accept("p", self.printp)
		#self.accept("r", self.printr)
	...
	def combat(self):
		...
		self.combat_sonatu.setSx(.02)
		self.combat_sonatu.setSy(.02)
		self.combat_sonatu.setSz(.02)
		self.combat_sonatu.setPos(16*20*sin(pi/3), 13*15, 0)
		self.combat_sonatu.lookAt(20*sin(pi/3)*7.5, -15*6.5, 0)
		self.combat_sonatu.reparentTo(self.render)
		sonatu = Sonatu(0, 0, 0, self.combat_sonatu, 1)
		...
		for j in range(6):
			...
			for i in range(16):
				gridspace_list.append(Gridspace(None, True, x_counter1, y_counter, hex_radius, j*i+i))
				x_counter1 = x_counter1 + 2*hex_radius*sin(pi/3)
			
			y_counter = y_counter - 3/2.0*hex_radius
			for i in range(15):
				gridspace_list.append(Gridspace(None, True, x_counter2, y_counter, hex_radius, j*i+i+16))
				x_counter2 = x_counter2 + 2*hex_radius*sin(pi/3)
			y_counter = y_counter - 3/2.0*hex_radius
			if j == 5: 
				x_counter1 = 0
				for i in range (16):
					gridspace_list.append(Gridspace(None, True, x_counter1, y_counter, hex_radius, (j+1)*i+i))
					x_counter1 = x_counter1 + 2*hex_radius*sin(pi/3)		
	...
	def combat_mouse_task(self, task):
		if base.mouseWatcherNode.hasMouse():
			self.accept("mouse1", self.printMouse )
		return Task.cont
	...
	def printMouse(self):
		if base.mouseWatcherNode.hasMouse():
			...
			self.combat_sonatu.setPos(self.PointAtZ(.5, self.near_point, self.near_vector))
			print self.mouse_position.getX()
			print self.mouse_position.getY()
